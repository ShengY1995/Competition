...
```{r}
feature_plot_df <- df_x_1 %>%
  mutate(outcome = df$outcome) %>%
  pivot_longer(cols=colnames(df_x_1))
```
```{r}
...
ggplot(rfTune_comp, aes(x = mtry, y = RMSE)) +
  ...
  annotate(geom="text", x=15, y=2.5, label="test1",
              color="blue")+
  annotate(geom="text", x=15, y=2.475, label="test2",
              color="green")+
  annotate(geom="text", x=15, y=2.54, label="comparison")+
  theme_classic()
